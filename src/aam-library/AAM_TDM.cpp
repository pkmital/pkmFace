...
void AAM_TDM::Train(const file_lists& pts_files, const file_lists& img_files, 
					const AAM_PAW& m_warp, int nPlane /* = 3 */, 
					double texture_percentage /* = 0.975 */,
					bool registration /* = true */)
{
	int nPoints = m_warp.nPoints();
	int nPixels = m_warp.nPix();
	int nSamples = pts_files.size();
	__nPlane = nPlane;
	
	CvMat *AllTextures = cvCreateMat(nSamples, nPixels*nPlane, CV_64FC1);
	...
	for(int i = 0; i < nSamples; i++)
	{
		...
		if(nPlane == 3) image = cvLoadImage(img_files[i].c_str(), CV_LOAD_IMAGE_COLOR);
		else image = cvLoadImage(img_files[i].c_str(), CV_LOAD_IMAGE_GRAYSCALE);
		...
		m_warp.CalcWarpTexture(matshape, image, &t, __nPlane);
...
void AAM_TDM::SaveSeriesTemplate(const CvMat* AllTextures, const AAM_PAW& m_warp)
{
	...
	if(access("registration", 0))	AAM_MKDIR("registration");
	if(access("Modes", 0))	AAM_MKDIR("Modes");
	if(access("Tri", 0))	AAM_MKDIR("Tri");
	char filename[100];
	...
	for(i = 0; i < AllTextures->rows; i++)
	{
		...
		m_warp.SaveWarpTextureToImage(filename, &oneTexture, __nPlane);
	}
	...
	for(int nmodes = 0; nmodes < nModes(); nmodes++)
	{
		...
		m_warp.SaveWarpTextureToImage(filename, &oneVar, __nPlane);
	}
	
	IplImage* templateimg = cvCreateImage
		(cvSize(m_warp.Width(), m_warp.Height()), IPL_DEPTH_8U, __nPlane);
	IplImage* convexImage = cvCreateImage
		(cvSize(m_warp.Width(), m_warp.Height()), IPL_DEPTH_8U, __nPlane);
	IplImage* TriImage = cvCreateImage
		(cvSize(m_warp.Width(), m_warp.Height()), IPL_DEPTH_8U, __nPlane);

	m_warp.SaveWarpTextureToImage("Modes/Template.jpg", __MeanTexture, __nPlane);
	m_warp.TextureToImage(templateimg, __MeanTexture, __nPlane);
...
{
	os << nPixels() << " " << nModes() << " " << nPlanes() << std::endl;
...
{	int _npixels, _nModes;
	is >> _npixels >> _nModes >> __nPlane;
	
	__MeanTexture = cvCreateMat(1, _npixels*__nPlane, CV_64FC1);
	__TextureEigenValues = cvCreateMat(1, _nModes, CV_64FC1);
	__TextureEigenVectors = cvCreateMat(_nModes, _npixels*__nPlane, CV_64FC1);
