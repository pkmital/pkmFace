...
public:
	...
	void Train(const file_lists& pts_files, const file_lists& img_files,
		...
		double texture_percentage = 0.975, bool registration  = true);
	...
	// Get number of pixels in texture model 
	inline const int nPixels()const{ return __MeanTexture->cols / __nPlane; }

	// Get number of modes of texture variation
	inline const int nModes()const{return __TextureEigenVectors->rows;}

	inline const int nPlanes()const {return __nPlane; }
