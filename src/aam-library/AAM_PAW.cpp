		...
		for(i = 0, k = 0; i < __nPixels; i++, k++)
		{
			...
			if(nchannel == 3)
			{
				...
				fastt[k] = 
					...
					s1*(t1*p1[ixR1]+t0*p2[ixR1])+s0*(t1*p1[ixR2]+t0*p2[ixR2]))/3;
			}
...
void AAM_PAW::Write(std::ofstream& os)
{
	...
	for(i = 0; i < (int)__vtri.size(); i++)
	{
		os << __vtri[i].size();
		for( j = 0; j < (int)__vtri[i].size(); j++)
		{
			os << " " << __vtri[i][j];
		}
		os << std::endl;
